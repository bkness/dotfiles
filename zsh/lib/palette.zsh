...
FILTER
  ...
  selected=$(
    ...
    | fzf "${FZF_THEME[@]}" \
        ...
        --color=label:#00ff00 \
        --prompt='  ❯ ' \
        --header='  Ctrl+P — browse  │  Ctrl+F — filter  │  Ctrl+G — reset' \
        --header-first \
        --with-nth=1 \
        --delimiter='│' \
        --bind "ctrl-f:execute(zsh '$_pick_script')+reload(zsh '$_filter_script')+change-header(  ■ PALETTE — filtered  │  Ctrl+G — reset all)" \
        --bind "ctrl-g:execute(printf '' > '$_cat_tmp')+reload(cat '$_pal_tmp')+change-header(  Ctrl+P — browse  │  Ctrl+F — filter  │  Ctrl+G — reset)" \
        --preview='
